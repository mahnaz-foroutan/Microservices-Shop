...
namespace Discount.Grpc.Repositories
{
    public class DiscountRepository : IDiscountRepository
    {
        ...
        public async Task<Coupon> GetDiscount(string productId)
        {
            ...
            if (coupon == null)
            {
                return new Coupon { ProductId ="" ,ProductName = "No Discount", Amount = 0, Description = "No Discount Description" };
            }
        ...
        public async Task<bool> UpdateDiscount(Coupon coupon)
        {
            ...
            var affected = await connection.ExecuteAsync
                ("UPDATE Coupon SET ProductId=@ProductId, ProductName=@ProductName, Description=@Description, Amount=@Amount WHERE id=@CouponId",
                new { ProductId=coupon.ProductId, ProductName = coupon.ProductName, Description = coupon.Description, Amount = coupon.Amount, CouponId = coupon.Id });
